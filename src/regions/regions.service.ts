...
export class RegionsService {
	...
	async validateRegion(code: string, type: 'province' | 'district' | 'ward') {
		switch (type) {
			case 'province':
				return await this.getProvinceByCode(code);
			case 'district':
				return await this.getDistrictByCode(code);
			case 'ward':
				return await this.getWardByCode(code);
			default:
				return null;
		}
	}
